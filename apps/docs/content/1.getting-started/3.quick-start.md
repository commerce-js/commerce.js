...
If you don't have an external eCommerce platform, use the built-in `@commercejs/platform` engine. It provides a Neon Postgres-powered commerce backend with 12 implemented domains and a full Admin API.
...
::tip
The platform connects to Neon Postgres via the `DATABASE_URL` environment variable. Use `seedDrizzle()` to populate with demo data for development.
::
