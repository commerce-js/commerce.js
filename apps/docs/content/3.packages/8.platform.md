---
title: "Platform"
description: Built-in commerce engine — SQLite or Neon Postgres, zero-config, batteries included.
navigation:
  icon: i-lucide-database
---

The `@commercejs/platform` package is the built-in CommerceJS commerce engine. It provides a fully functional eCommerce backend powered by SQLite (local) or Neon Postgres (cloud) — zero external APIs, zero configuration.
...
// Create the adapter (async — auto-detects driver from DATABASE_URL)
...
const commerce = createCommerce({
  adapter: await createPlatformAdapter({ currency: 'SAR' }),
})
...
The `createPlatformAdapter()` function is **async** and accepts these options:

| Option | Type | Required | Default | Description |
|---|---|---|---|---|
| `currency` | `string` | No | `'SAR'` | Store currency code for price formatting |
| `driver` | `'sqlite' \| 'neon'` | No | Auto-detected | Database driver to use |
| `connectionString` | `string` | No | `DATABASE_URL` env | Neon connection string |

## Database Drivers

The platform supports three database drivers:
...
| `neon` | Prisma + `@prisma/adapter-neon` | Cloud / edge deployment (Cloudflare Workers) |

```ts
// Drizzle (default — SQLite)
initDatabase({ driver: 'drizzle' })
...
Both SQLite drivers share identical query interfaces and produce identical results.

### Neon Postgres (Cloud)

For cloud deployments, the adapter auto-detects Neon from `DATABASE_URL`:

```ts
// Auto-detection: postgres:// URLs → Neon, otherwise → SQLite
process.env.DATABASE_URL = 'postgres://user:pass@ep-xxx.neon.tech/commerce'
const adapter = await createPlatformAdapter({ currency: 'USD' })
// → Automatically uses Neon Postgres
```
...
Neon requires optional peer dependencies:
