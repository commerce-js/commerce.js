...
// ---------------------------------------------------------------------------
// /admin/reset/:token — admin password-reset completion (pre-auth)
// ---------------------------------------------------------------------------
// On mount validates the token via GET /api/admin/reset/:token. On success
// shows a "new password + confirm" form; on 404 shows "link invalid". POST
// /complete issues the merchant session in the same response so the admin
// lands at /admin signed in.
// ---------------------------------------------------------------------------
...
onMounted(async () => {
  ...
  try {
    const res = await $fetch<{ email: string, expiresAt: string }>(
      `/api/admin/reset/${encodeURIComponent(token.value)}`,
      { credentials: 'include' },
    )
...
async function handleSubmit() {
  ...
  try {
    await $fetch(`/api/admin/reset/${encodeURIComponent(token.value!)}/complete`, {
      method: 'POST',
      credentials: 'include',
    ...
    <UCard class="w-full max-w-md">
      ...
      <div v-else-if="status === 'invalid'" class="space-y-4">
        <UAlert
          ...
          description="It may have expired (links work for 1 hour), already been used, or been copied incorrectly. Request a new one to continue."
        />
        <UButton color="primary" to="/admin/forgot-password" block>
          Request a new link
        </UButton>
      </div>
