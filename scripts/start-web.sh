...
#
#   tini is PID 1 (see Dockerfile ENTRYPOINT) and reaps zombies. This
#   script traps SIGTERM/SIGINT and forwards them to both children. When
#   either child exits, we tear the other down and exit non-zero — that
#   triggers Fly's restart policy so the whole machine gets rebuilt,
#   keeping the two processes "all-or-nothing" from Fly's perspective.
# ---------------------------------------------------------------------------
set -eu
...
# Dashboard already defaults to listening on $HOST:$PORT = 0.0.0.0:3000.
...
# binds to 3001 instead of inheriting 3000. Env is set inline so it
# doesn't leak into the dashboard process launched above.
...
# Wait for EITHER child to exit. `wait -n` is in POSIX sh / dash (Debian
# default on node:22-slim). The first child to die triggers a full
# teardown so Fly restarts the machine and we don't limp along in a
# split-brain state.
