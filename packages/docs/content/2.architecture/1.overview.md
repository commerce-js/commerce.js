...
graph TD
    ...
    C --> F
    E --> F
    H --> G["@commercejs/storefront"]
```

All packages depend on `@commercejs/types` as the shared language. The `@commercejs/core` engine orchestrates adapters, payment providers, and events into a single entry point. The checkout engine and hosted checkout handle the payment flow.
