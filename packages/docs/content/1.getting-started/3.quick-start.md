...
flowchart TD
    ...
    idle ==>|setCustomerInfo| info[info]
    info ==>|setShippingAddress| shipping[shipping]
    shipping ==>|submitPayment| payment[payment]
    payment ==>|confirmPayment| confirming[confirming]
    confirming ==>|captured| complete([Complete])
    confirming -.->|failed| failed([Failed])
    payment ==>|webhook CAPTURED| complete
    payment -.->|webhook DECLINED| failed
    failed -->|retry| payment

    classDef state fill:#3b82f6,stroke:#2563eb,color:#fff
    classDef success fill:#10b981,stroke:#059669,color:#fff
    classDef error fill:#ef4444,stroke:#dc2626,color:#fff
    classDef action fill:#818cf8,stroke:#6366f1,color:#fff

    class idle,info,shipping state
    class payment,confirming action
