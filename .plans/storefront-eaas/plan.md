...
- **2026-04-16**: **Carry-over #1 fixed — hosted checkout back online for card payments.** `apps/hosted-checkout` ships as a third co-supervised Fly process on `:3002`, routed from `checkout.commercejs.cloud` via the dashboard's proxy middleware. Migrated off Cloudflare: `nuxt.config.ts` preset → `node-server` + `buildAssetsDir: '/_checkout/'`, removed `wrangler` devDep, deleted `dist/` + `.wrangler/` CF artifacts. New `server/middleware/tenant.ts` resolves merchant from `merchant` query param or `cjs_merchant` cookie via raw SQL against the control DB (no second Prisma schema — one `SELECT` doesn't justify it), new `server/utils/control-db.ts` wraps that call. All 13 API routes had their legacy `ensureDb()` calls removed; tenant DB binding now happens per-request. Storefront's card-payment redirect in `checkout.vue` extracts the merchant subdomain from `window.location.hostname` and appends `&merchant=<subdomain>` to the hosted-checkout URL. Infra: `scripts/start-web.sh` now supervises 3 processes; `00.storefront-proxy.ts` adds a `CHECKOUT_HOSTS` check that proxies ALL paths (including `/api/*`) to `:3002` — different from the storefront pattern because the checkout's own API routes need tenant-scoped DB context; `fly.toml` adds `CHECKOUT_PORT` + `CHECKOUT_ORIGIN`; Dockerfile copies + builds hosted-checkout. **Important architectural addition**: `@commercejs/platform` grew a new `registerEventResolver()` export — a framework-agnostic way to teach `getDb()` how to read a per-request Prisma client off `event.context.db`. The tenant middleware sets `event.context.db = getPrismaClient(merchant.database_url)`; the platform's `getDb()` reads it via the registered resolver. This is the scalable concurrency-safe answer to multi-tenant DB binding — it supersedes the brittle `bindDb()` + `AsyncLocalStorage.enterWith()` path that doesn't propagate reliably across Nitro's middleware→handler boundary. Hosted-checkout uses this pattern; dashboard will be migrated in a follow-up. Requires `nitro.experimental.asyncContext: true` to enable `useEvent()`. Deployed to Fly, browser verified: full card-payment flow (home → cart → /checkout with card → redirected to checkout.commercejs.cloud → cart loads → Tap form renders).
...
- **2026-04-16 (card-payment follow-ups, commits `c4b4644` + `70accb9`)**: Two late-breaking fixes on the card-payment path surfaced by browser testing after carry-over #1 landed. (1) **Shipping total math**: `platform/domains/cart.ts` had `shippingRates.standard = 20` but `checkout.ts::getShippingMethods` returned `standard: 15` — a 5 SAR drift between the selected option and the total. Aligned both to 15 and TODO'd the duplication for a shared source of truth. Also made `storefront/checkout.vue`'s `total` computed recompute locally from `subtotal + selectedShipping.price` so the summary reacts to the radio click, not just to the Continue-to-Payment PATCH; after the PATCH the server-persisted total matches exactly. (2) **Tap card form + Tap payment submit**: both blocked by the `NUXT_*` runtime-config convention. `runtimeConfig.public.tapPublicKey` needed `NUXT_PUBLIC_TAP_PUBLIC_KEY` as a Fly secret (plain `TAP_PUBLIC_KEY` is only readable at build time); `runtimeConfig.tapSecretKey` needed `NUXT_TAP_SECRET_KEY` (same class of bug, server-side). Both secrets set via `fly secrets set` — no code redeploy for the secrets themselves. Improved the error message in `hosted-checkout/server/utils/tap.ts::useTapProviderFromEnv` so the next person is pointed at the right env var name instead of the misleading unprefixed one. This is the third `NUXT_*` prefix gotcha across two sessions (Google Maps, Tap public, Tap secret). Browser-verified: card-payment flow now reaches Tap's API end-to-end — a fake token curl returns Tap's "Source already used" 400 (correct), meaning the plumbing works and a real tokenized card would complete. Full COD + card paths both live on `smoke.commercejs.cloud`.
<!-- META_INFORMATION -->
