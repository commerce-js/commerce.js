...
─────────────────────────────────────────────────────────
  ...
  Current blocker:       T01 live acceptance complete on smoke — 6 of
                         9 scenarios green via API, 1 pending browser
                         check (scenario 8: no Reka console errors),
                         1 gated on dev-DB having null-actor rows
                         (scenario 7 — production is clean so this
                         exercises on first system action, code path
                         unchanged), 1 gated on release CI (scenario 9
                         storage-s3 0.2.1 publish — rides T05). One
                         real gap found + fixed inline: the category
                         PATCH route wasn't wrapping platform throws
                         as 400, fixed + redeployed in commit
                         `528867a`. Next workstream is T02
                         transactional emails, sub-plan already
                         spawned at `.plans/transactional-emails/
                         plan.md` (T02 row on eaas-launch flipped 🟡
                         → 🟢 by the parallel session on 2026-04-20).
...
- **2026-04-20** — T01 deployed + live acceptance on smoke.commercejs.cloud. Two rollouts to `commercejs-cloud.fly.dev` (4 machines × 2 deploys, all green). Acceptance via curl with `SMOKE_MERCHANT_EMAIL`/`SMOKE_MERCHANT_PASSWORD` from `.secrets`: baseline health + 6 scenarios green (bcrypt async login 657ms warm; category products-attached guard 400 "4 attached products"; self-parent guard 400; descendant-cycle guard 400; `sortOrder` surfaced on categories + nested `product.categories`; orphan actorId present in activity feed, chip render path trivially covered). Scenarios not exercisable from API/dev-DB: (7) no null-actor rows on current smoke DB, production will be clean and exercise on first system action; (8) browser-only console-error sweep pending owner verification; (9) storage-s3 0.2.1 still 0.2.0 on npm, publish rides T05 branch-swap or an explicit `pnpm changeset publish` from a release-capable branch. **One gap surfaced + fixed inline**: `apps/dashboard/server/api/admin/categories/[id].patch.ts` was returning 500 instead of 400 for the new platform guard throws (self-parent + descendant-cycle); the sibling `[id].delete.ts` already had the try/catch wrapper but PATCH didn't. Fixed and redeployed as commit `528867a`; rerun of scenarios 3 + 4 confirms clean 400 with the platform's message preserved. T01 Execution Summary in `.plans/eaas-launch/tasks/T01.md` updated with the full acceptance table + follow-up note. eaas-launch Change Log entry added. State Snapshot "Current blocker" rewritten from "awaiting deploy" to "acceptance done, T02 in flight (sub-plan spawned by parallel session)".
